...
// This component receives a FROZEN copy of data from db.getFriendProfile()
// There are no inputs, no save buttons, no way to mutate anything.
export function friendProfile(friendId, viewerUsername) {
  const profile = db.getFriendProfile(friendId);
  ...
  if (!profile) {
    return `
      <div class="max-w-2xl mx-auto px-4 py-20 text-center fade-up">
        ...
        <p style="color:var(--muted)" class="mb-6">${friendId} hasn't logged any data yet.</p>
        <button onclick="navigateTo('friends')" class="btn-ghost px-6 py-2 rounded-lg">← Back to Friends</button>
      </div>
    `;
  }
  ...
  // All-time stats
  const allWorkouts = Object.values(allLogs).reduce((sum, l) => sum + l.workouts.length, 0);
  const totalSteps  = Object.values(allLogs).reduce((sum, l) => sum + (l.steps || 0), 0);
  const activeDays  = Object.keys(allLogs).filter(d =>
    allLogs[d].workouts.length > 0 || allLogs[d].steps > 0
  ).length;
 
  // Recent activity (last 5 workouts across all days, read-only)
  ...
  return `
    <div class="max-w-3xl mx-auto px-4 py-10 fade-up">
      ...
      <div style="background:linear-gradient(135deg,#1e2a0e,#141414);border:1px solid #2a2a2a;"
        ...
        <div>
          <h2 class="bebas text-4xl mb-1">${friendId}</h2>
      ...
      <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <div class="dark-card p-5 text-center">
          <p class="bebas text-4xl" style="color:var(--accent)">${totalSteps > 0 ? (totalSteps / 1000).toFixed(1) + "k" : "—"}</p>
